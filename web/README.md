...
corresponde a las fases **F0** (scaffold), **F1** (landing), **F2** (onboarding), **F3**
(sesión + dashboard) y **F4** (panel de flujos) del plan de migración:
...
- Los estilos se inlinean por defecto (comportamiento `inlineStylesheets: 'auto'` de Astro);
  el dashboard importa `static/css/style.css` y el panel admin además
  `static/css/admin_flows.css`, ambos por el pipeline de Vite (URL hasheada en `/_astro`).
